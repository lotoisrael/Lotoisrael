...
function renderResults(data) {
  ...
  document.getElementById('results-card').innerHTML =
    `<h2 id="results-heading" class="results-heading">תוצאות הגרלת לוטו מס׳ ${data.drawNumber || '—'}</h2>
     <p class="results-date"><time>${data.date || ''}</time></p>
     <div class="results-row" role="group">
       <div class="main-balls" role="list">${nums.map((n, i) => `<div class="ball ball-main" role="listitem" aria-label="מספר ${i + 1}: ${n}">${n}</div>`).join('')}</div>
       <div class="strong-wrap">
         <div class="ball ball-strong" role="img" aria-label="מספר חזק ${data.strongNumber || '?'}">${data.strongNumber || '?'}</div>
...
function _renderBalls(showReasons) {
  const rb = document.getElementById('result-balls');
  rb.innerHTML = '';
  _nums.forEach((n, i) => {
    const b = document.createElement('div');
    b.className = 'rball rball-main'; b.textContent = String(n);
    b.setAttribute('role', 'listitem'); b.setAttribute('aria-label', `מספר ${i + 1}: ${n}`);
    b.style.animationDelay = i * .08 + 's'; rb.appendChild(b);
  });
  const pl = document.createElement('span'); pl.className = 'plus-sep'; pl.textContent = '+'; pl.setAttribute('aria-hidden', 'true'); rb.appendChild(pl);
  const sb = document.createElement('div');
  sb.className = 'rball rball-strong'; sb.textContent = String(_strong);
  sb.setAttribute('role', 'listitem'); sb.setAttribute('aria-label', `מספר חזק: ${_strong}`);
  sb.style.animationDelay = '.55s'; rb.appendChild(sb);
  ...
  _nums.forEach((n, i) => {
    ...
    if (!showReasons) {
      [n, _strong].forEach(num => {
        content = content.replace(new RegExp('(?<![\\u0590-\\u05FF\\w])' + num + '(?![\\u0590-\\u05FF\\w])', 'g'), '<span class="num-mask" aria-hidden="true">??</span>');
      });
    }
    const c = document.createElement('div'); c.className = 'why-card';
    c.setAttribute('aria-label', `מספר ${n} – ${_reasons[i] || ''}`);
    c.innerHTML = `<div class="why-ball-row"><div class="why-mini" aria-hidden="true">${showReasons ? n : '?'}</div></div><p class="why-reason">${content}</p><div class="why-bar" aria-hidden="true"><div class="why-fill" data-w="${pct}"></div></div>`;
    wg.appendChild(c);
  });
}
...
function checkReturnFromStripe() {
  ...
  if (Date.now() - p.ts > EXPIRY_MS) { localStorage.removeItem(PENDING_KEY); alert('⏰ פג תוקף הנתונים. אנא צור שורה חדשה.'); return; }
...
function makeTrap(sel, closeFn) {
  return e => {
    ...
    const els = [...m.querySelectorAll('a[href],button:not([disabled]),[tabindex]:not([tabindex="-1"])')];
...
let accTrap = null, payTrap = null, termsTrap = null;

function openAccModal(e) { if (e) e.preventDefault(); lastFocus = document.activeElement; document.getElementById('acc-modal-bg').classList.add('open'); accTrap = makeTrap('#acc-modal-bg .modal', closeAccModal); document.addEventListener('keydown', accTrap); setTimeout(() => { const f = document.querySelector('#acc-modal-bg .modal button'); if (f) f.focus(); }, 50); }
function closeAccModal() { document.getElementById('acc-modal-bg').classList.remove('open'); document.removeEventListener('keydown', accTrap); if (lastFocus) lastFocus.focus(); }
...
function closePayModal() { document.getElementById('modal-bg').classList.remove('open'); document.removeEventListener('keydown', payTrap); if (lastFocus) lastFocus.focus(); }
function payBgClose(e) { if (e.target === document.getElementById('modal-bg')) closePayModal(); }

function openTermsModal(e) { if (e) e.preventDefault(); lastFocus = document.activeElement; document.getElementById('terms-modal-bg').classList.add('open'); termsTrap = makeTrap('#terms-modal-bg .modal', closeTermsModal); document.addEventListener('keydown', termsTrap); setTimeout(() => { const f = document.querySelector('#terms-modal-bg .modal button'); if (f) f.focus(); }, 50); }
function closeTermsModal() { document.getElementById('terms-modal-bg').classList.remove('open'); document.removeEventListener('keydown', termsTrap); if (lastFocus) lastFocus.focus(); }

// ─── Terms Modal Builder ──────────────────────────────────────
(function () {
  const bg = document.createElement('div');
  bg.id = 'terms-modal-bg'; bg.className = 'modal-bg';
  bg.setAttribute('role', 'dialog'); bg.setAttribute('aria-modal', 'true'); bg.setAttribute('aria-labelledby', 'terms-title');
  bg.innerHTML = `<div class="modal acc-modal" style="max-width:540px"><div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:12px;border-bottom:2px solid #f5c0c8;padding-bottom:12px"><h2 id="terms-title" style="font-size:clamp(16px,3vw,20px);font-weight:900;color:#c8102e">📋 תנאי שימוש</h2><button onclick="closeTermsModal()" aria-label="סגור" style="background:none;border:none;font-size:22px;cursor:pointer;color:#333;padding:4px 8px">✕</button></div><p style="font-size:11px;color:#555;margin-bottom:14px">עודכן לאחרונה: מרץ 2026</p><div style="display:flex;flex-direction:column;gap:16px;font-size:12px;color:#222;line-height:1.9;text-align:right">
<div><h3 style="font-size:13px;font-weight:900;color:#c8102e;margin-bottom:4px">1. כללי</h3><p>האתר <strong>לוטו ישראל</strong> מופעל באופן עצמאי ואינו קשור למפעל הפיס, אינו מייצגו ואינו פועל מטעמו בשום צורה. השימוש באתר מהווה הסכמה מלאה לתנאי שימוש אלה.</p></div>
<div><h3 style="font-size:13px;font-weight:900;color:#c8102e;margin-bottom:4px">2. מטרת השירות ואזהרת הגרלות</h3><p>האתר מספק כלי <strong>לבידור בלבד</strong> ולניתוח סטטיסטי. המספרים המופקים אינם מבטיחים זכייה, אינם מגדילים את סיכויי הזכייה ואינם מהווים המלצה כלשהי. הגרלת הלוטו היא משחק מזל טהור. ניתוח סטטיסטי אינו משנה את הסיכויים האקטואריים.</p></div>
<div><h3 style="font-size:13px;font-weight:900;color:#c8102e;margin-bottom:4px">3. גיל מינימלי</h3><p>השימוש מותר לבני <strong>18 ומעלה בלבד</strong> בהתאם לחוק הישראלי. קטינים אינם רשאים לעשות שימוש בשירותי האתר.</p></div>
<div><h3 style="font-size:13px;font-weight:900;color:#c8102e;margin-bottom:4px">4. משחק אחראי</h3><p>אם אתם חשים שהמשחק משפיע לרעה על חייכם, פנו למוקד הטיפול בהתמכרויות: <strong>1-800-599-029</strong> (שירות חינמי של משרד הבריאות).</p></div>
<div><h3 style="font-size:13px;font-weight:900;color:#c8102e;margin-bottom:4px">5. תשלומים והחזרים</h3><p>התשלומים מתבצעים דרך PayPal. לאחר חשיפת המספרים <strong>לא ניתן לבצע החזר כספי</strong> מאחר שהשירות סופק במלואו. לבעיות טכניות פנה תוך 48 שעות.</p></div>
<div><h3 style="font-size:13px;font-weight:900;color:#c8102e;margin-bottom:4px">6. הגבלת אחריות</h3><p>החברה לא תישא בכל אחריות לנזק ישיר, עקיף או תוצאתי הנובע מהסתמכות על תוצאות האתר, לרבות הפסדים כספיים בהגרלות.</p></div>
<div><h3 style="font-size:13px;font-weight:900;color:#c8102e;margin-bottom:4px">7. קניין רוחני</h3><p>כל התכנים, העיצוב והאלגוריתמים מוגנים בזכויות יוצרים. אין להעתיק, לשכפל או להפיץ ללא אישור בכתב.</p></div>
<div><h3 style="font-size:13px;font-weight:900;color:#c8102e;margin-bottom:4px">8. פרטיות ועוגיות</h3><p>האתר משתמש ב-Google Analytics לניתוח תנועה אנונימי בלבד. לא נאסף מידע אישי מזהה.</p></div>
<div><h3 style="font-size:13px;font-weight:900;color:#c8102e;margin-bottom:4px">9. שינויים בתנאים</h3><p>החברה רשאית לעדכן תנאים בכל עת. המשך השימוש מהווה הסכמה לתנאים המעודכנים.</p></div>
<div><h3 style="font-size:13px;font-weight:900;color:#c8102e;margin-bottom:4px">10. דין וסמכות שיפוט</h3><p>תנאי שימוש אלה כפופים לדיני מדינת ישראל. כל סכסוך יובא לבית המשפט במחוז תל אביב-יפו.</p></div>
<div><h3 style="font-size:13px;font-weight:900;color:#c8102e;margin-bottom:4px">11. יצירת קשר</h3><p>לשאלות: <a href="mailto:Info@lotoisrael.com" style="color:#c8102e;font-weight:700">Info@lotoisrael.com</a></p></div>
</div><button class="acc-close-btn" onclick="closeTermsModal()" style="margin-top:18px">סגור</button></div>`;
  bg.addEventListener('click', e => { if (e.target === bg) closeTermsModal(); });
